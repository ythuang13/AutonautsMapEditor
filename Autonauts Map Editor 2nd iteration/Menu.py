...
class Menu:
    ...
    def toolbarTabSetup(self):
        toolsLayout = [[sg.Combo(["brush", "bucket"], key="-toolsSelect-", default_value="brush", readonly=True), sg.Button("fill", tooltip="fill the whole map with one tile", k="-fill-", size=BUTTON_SIZE), sg.Button("reset", tooltip="reset back to original", k="-reset-", size=BUTTON_SIZE), sg.Button("Undo", tooltip="undo one time", key="-undo-", disabled=True, size=BUTTON_SIZE), sg.Button("Redo", tooltip="redo one time", key="-redo-", disabled=True, size=BUTTON_SIZE)],
                [sg.Text("Brush Size:"), sg.Slider(range=(1, 10), default_value=2, orientation="h", k="-brushSize-", relief="raised", size=(15, 15)), sg.Button("Screenshot", tooltip="save a screenshot to your desktop", k="-screenshot-")]
                ]
    ...
    def settingsTabSetup(self):
        setting = [[sg.Button("load", k="-load-", size=BUTTON_SIZE), sg.Button("template", k="-template-"), sg.Button("export", disabled=True, k="-export-", size=BUTTON_SIZE) , sg.Button("help", k="-help-", size=BUTTON_SIZE), sg.Button("quick launch", k="-quick-")],
        ...
        theme_swap = [[sg.Combo(THEMES, key="-theme-", default_value=self.theme, readonly=True)],[sg.Button("submit", k="-themeSubmit-")]]
